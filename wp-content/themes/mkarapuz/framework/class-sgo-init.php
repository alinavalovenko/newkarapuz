	...
	if ( ! class_exists( 'KARAPUZ_Init' ) ) {
		...
		class KARAPUZ_Init {
			...
			function register_menus_area() {
				register_nav_menus(
					array (
						'primary-menu' => esc_html__( 'Primary Menu', 'mKARAPUZ_' ),
						'mobile-menu'  => esc_html__( 'Mobile Menu', 'mKARAPUZ_' ),
						'footer-menu'  => esc_html__( 'Footer Menu', 'mKARAPUZ_' ),
					)
				);
			}
			...
			function KARAPUZ_add_customizer_sett_section( $wp_customize ) {
				$wp_customize->add_section( 'KARAPUZ_theme_settings', array (
					'priority' => 10,
					'title'    => 'SGO Theme Settings',
				) );

				$wp_customize->add_setting( 'KARAPUZ_theme_settings[logo]', array (
					'transport'     => 'refresh',
					'height'        => 180,
					'width'         => 160,
				...
				$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'KARAPUZ_theme_settings', array (
					'label'    => __( 'SGO Logo', 'mKARAPUZ_' ),
					'description' => __('Uploud custom image','mKARAPUZ_'),
					'section'  => 'KARAPUZ_theme_settings',
					'settings' => 'KARAPUZ_theme_settings[logo]',
				) ) );
			}
		}
