...
define( 'INSIGHT_SWATCHES_VERSION', '1.0.4' );
define( 'INSIGHT_SWATCHES_URL', plugin_dir_url( __FILE__ ) );
define( 'INSIGHT_SWATCHES_PATH', plugin_dir_path( __FILE__ ) );

if ( ! class_exists( 'Insight_Swatches' ) ) {
	class Insight_Swatches {
		function __construct() {
			add_action( 'init', array( $this, 'init' ) );
			add_action( 'woocommerce_variable_add_to_cart', array( $this, 'variable_scripts' ) );
		}
		...
		public function init() {
			...
			load_plugin_textdomain( 'insight-swatches', false, basename( dirname( __FILE__ ) ) . '/languages/' );
		...
		function option_terms( $tax, $i ) {
			...
			if ( 'select' !== $tax->attribute_type ) {
				...
				<select multiple="multiple"
				        data-placeholder="<?php esc_attr_e( 'Select terms', 'insight-swatches' ); ?>"
				...
				<button class="button plus select_all_attributes">
					<?php esc_html_e( 'Select all', 'insight-swatches' ); ?>
				</button>
				<button class="button minus select_no_attributes">
					<?php esc_html_e( 'Select none', 'insight-swatches' ); ?>
				</button>
				<button class="button fr plus add_new_attribute">
					<?php esc_html_e( 'Add new', 'insight-swatches' ); ?>
				</button>
				<?php
			}
		}
		...
		public function scripts() {
			...
			wp_enqueue_script( 'isw-frontend', INSIGHT_SWATCHES_URL . 'assets/js/scripts.js', array( 'jquery' ), INSIGHT_SWATCHES_VERSION, true );
